...
void CreateShaderCode()
{
		...
		for (dirIter; dirIter != dirIterEnd; ++dirIter)
		{
			...
			if (false == file.is_directory())
			{
				...
				for (int i = 0; i < (int)eSHADER_TYPE::END; ++i)
				{
					...
					{
						//Prefix를 제거
						SFileNameBase.erase(0, strPos + 1);
						flagPipeline |= 1 << i;
						break;
					}
				}
	...
	for (const auto& mapIter : mapShaderInfo)
	{
		...
		else
		{
			...
			string msg = "New Shader Pipeline Detected.\n Path: ";
			msg += JsonPath.string();
			msg += "\nAdjust values and click OK Button.";
			//메시지 박스를 띄워서 프로그램을 일시정지 시킨 뒤 
			MessageBoxA(nullptr, msg.c_str(), "Notification", MB_OK);

			//파일을 다시 열어서 데이터를 다시 가져온 후 닫는다.
			ifstream ifs(JsonPath);
			if (false == ifs.is_open())
			{
				ERROR_MESSAGE("Failed to open shader pipeline json file!!");
			}
		...
		//"""""자동 생성된 정규화 쉐이더 파이프라인일 경우""""에만 쉐이더가 추가된 것이 있는지 확인한다.
		if (false == JsonInfo.isMember(RES_INFO::SHADER::GRAPHICS::JSON_KEY::bIsAutoGenerated))
		{
			throw(std::runtime_error("Var bIsAutoGenerated Missing!!!"));
		}
		
		//자동생성된 코드인지 여부를 확인
		bool bIsAutoGenerated = JsonInfo[RES_INFO::SHADER::GRAPHICS::JSON_KEY::bIsAutoGenerated].asBool();
		...
		if (bIsAutoGenerated)
		{
			...
			int origFlag = JsonInfo[RES_INFO::SHADER::GRAPHICS::JSON_KEY::eSHADER_PIPELINE_STAGE].asInt();
			if (mapIter.second.flagPipelineStage != origFlag)
			{
				ofstream fpJsonOut(JsonPath);
				if (true == fpJsonOut.is_open())
				{
					JsonInfo[RES_INFO::SHADER::GRAPHICS::JSON_KEY::eSHADER_PIPELINE_STAGE] = mapIter.second.flagPipelineStage;
