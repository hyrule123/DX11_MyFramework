...
private:
    FSM() = delete;
public:
    //strKey -> Script로 전달, _uStateID -> FSM으로 전달
    FSM(const string_view _strKey, UINT _uStateID);

    FSM(const FSM& _other);
    virtual FSM* Clone() = 0;
...
public:
    virtual void SetHolder(ScriptHolder* _pScriptHolder) override;
...
inline eFSM_RESULT FSM::Transition(const tFSM_Event& _tEvent)
{
