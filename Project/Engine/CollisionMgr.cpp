...
void CollisionMgr::CalcSimpleCollGrid2D(__in Collider2D* _pCol, __in Vec4 _vLBRTPos, __out vector<UINT>& _vecIdx)
{
	_vecIdx.clear();
...
void CollisionMgr::AddCollider2D(Collider2D* _pCol, const vector<UINT>& _vecIdx)
{
...
void CollisionMgr::Create2DGrid(Vec2 _vWorldLB, Vec2 _vWorldSize, UINT _uiGridNumX, UINT _uiGridNumY)
{
...
void CollisionMgr::tick()
{
	...
	for (int i = 0; i < m_iNum2DGridTotalIndex; ++i)
	{
		...
		std::sort(m_vec2DGrid[i].vecColl.begin(), m_vec2DGrid[i].vecColl.end(),
			[](Collider2D* _pColA, Collider2D* _pColB)->bool
			{
		...
		for (int l = 0; l < size; ++l)
		{
			for (int m = l + 1; m < size; ++m)
			{
				...
				//만약 삭제 대기 상태일 경우 - GameObject에서 tick과 finaltick을 돌리지 않음
				...
				Collider2D* pColA = m_vec2DGrid[i].vecColl[l];
				Collider2D* pColB = m_vec2DGrid[i].vecColl[m];
				...
				if (true == CheckCollision2D(pColA, pColB, HitPoint))
				{
					...
					else
					{
						...
						iter->second.llCheckedCount = TimeMgr::GetInst()->GetCurCount();
					}
	...
	//이번 틱에 기록된 진동값을 받아온다.
	LONGLONG CurTime = TimeMgr::GetInst()->GetCurCount();
...
bool CollisionMgr::CheckCollision2D(Collider2D* _pCol_1, Collider2D* _pCol_2, Vec2& _v2HitPoint)
{
	return m_arrFuncCheckCollision2D[(int)_pCol_1->GetColliderType()][(int)_pCol_2->GetColliderType()](_pCol_1, _pCol_2, _v2HitPoint);
}

bool CollisionMgr::CheckCollision2D_Rect_Rect(Collider2D* _pColRect_1, Collider2D* _pColRect_2, Vec2& _v2HitPoint)
{
...
bool CollisionMgr::CheckCollision2D_Rect_Circle(Collider2D* _pColRect, Collider2D* _pColCircle, Vec2& _v2HitPoint)
{
	enum LBRT { L, B, R, T };
	const Vec4& RectLBRT = _pColRect->GetSimpleCollider();
	const Vec2& CircleCenter = _pColCircle->GetCenterPos().XY();
	float CircleRadius = static_cast<Collider2D_Circle*>(_pColCircle)->GetRadius();
...
bool CollisionMgr::CheckCollision2D_Rect_OBB(Collider2D* _pColRect, Collider2D* _pColOBB, Vec2& _v2HitPoint)
{
	return false;
}

bool CollisionMgr::CheckCollision2D_Rect_Point(Collider2D* _pColRect, Collider2D* _pColPoint, Vec2& _v2HitPoint)
{
...
bool CollisionMgr::CheckCollision2D_Rect_Point_CollInfo(const Vec4& _v4LBRT, const Vec2& _v2Point)
{
	enum LBRT { L, B, R, T };
...
bool CollisionMgr::CheckCollision2D_Circle_Circle(Collider2D* _pColCircle_1, Collider2D* _pColCircle_2, Vec2& _v2HitPoint)
{
	...
	float RadiusSum = static_cast<Collider2D_Circle*>(_pColCircle_1)->GetRadius();
	RadiusSum += static_cast<Collider2D_Circle*>(_pColCircle_2)->GetRadius();
	RadiusSum *= RadiusSum;
...
bool CollisionMgr::CheckCollision2D_Circle_OBB(Collider2D* _pColCircle, Collider2D* _pColOBB, Vec2& _v2HitPoint)
{
	return false;
}

bool CollisionMgr::CheckCollision2D_Circle_Point(Collider2D* _pColCircle, Collider2D* _pColPoint, Vec2& _v2HitPoint)
{
	const Vec2& CenterPos = _pColCircle->GetCenterPos().XY();
	const Vec2& Point = _pColPoint->GetCenterPos().XY();

	float fRad = static_cast<Collider2D_Circle*>(_pColCircle)->GetRadius();
...
bool CollisionMgr::CheckCollision2D_Circle_Point_CollInfo(const Vec2& _v2CircleCenterPos, float _fCircleRadius, const Vec2& _v2PointPos)
{
...
bool CollisionMgr::CheckCollision2D_OBB_OBB(Collider2D* _pColOBB2D_1, Collider2D* _pColOBB2D_2, Vec2& _v2HitPoint)
{
	const tOBB2D& OBB_1 = static_cast<Collider2D_OBB*>(_pColOBB2D_1)->GetOBBInfo();
	const tOBB2D& OBB_2 = static_cast<Collider2D_OBB*>(_pColOBB2D_2)->GetOBBInfo();
...
bool CollisionMgr::CheckCollision2D_OBB_Point(Collider2D* _pColOBB2D, Collider2D* _pColPoint, Vec2& _v2HitPoint)
{
	const tOBB2D& OBBInfo = static_cast<Collider2D_OBB*>(_pColOBB2D)->GetOBBInfo();
...
bool CollisionMgr::CheckCollision2D_Point_Point(Collider2D* _pColPoint_1, Collider2D* _pColPoint_2, Vec2& _v2HitPoint)
{
...
UINT CollisionMgr::ComputeRelativePos_Rect_Point(const Vec4& _v4RectLBRT, const Vec2& _v2Point)
{
	enum LBRT { L, B, R, T };
...
void CollisionMgr::ComputeHitPointByRectLBRT(const Vec4& _v4LBRT_1, const Vec4& _v4LBRT_2, Vec2& _v2HitPoint)
{
	enum LBRT { L, B, R, T };
...
void CollisionMgr::RegisterCollisionFunc()
{
	...
	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::CIRCLE][(int)eCOLLIDER_TYPE_2D::CIRCLE]
		= std::bind(&CollisionMgr::CheckCollision2D_Circle_Circle, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);

	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::CIRCLE][(int)eCOLLIDER_TYPE_2D::OBB]
		= std::bind(&CollisionMgr::CheckCollision2D_Circle_OBB, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);

	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::CIRCLE][(int)eCOLLIDER_TYPE_2D::POINT]
		= std::bind(&CollisionMgr::CheckCollision2D_Circle_Point, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);
	...
	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::OBB][(int)eCOLLIDER_TYPE_2D::OBB]
		= std::bind(&CollisionMgr::CheckCollision2D_OBB_OBB, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);

	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::OBB][(int)eCOLLIDER_TYPE_2D::POINT]
		= std::bind(&CollisionMgr::CheckCollision2D_OBB_Point, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);
	...
	m_arrFuncCheckCollision2D[(int)eCOLLIDER_TYPE_2D::POINT][(int)eCOLLIDER_TYPE_2D::POINT]
		= std::bind(&CollisionMgr::CheckCollision2D_Point_Point, this, 
			std::placeholders::_1, std::placeholders::_2, 
			std::placeholders::_3);
}
