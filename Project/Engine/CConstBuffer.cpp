...
void CConstBuffer::BindBuffer(UINT _eSHADER_PIPELINE_FLAG)
{
	...
	if (def_Shader::eSHADER_PIPELINE_STAGE::__NONE == _eSHADER_PIPELINE_FLAG)
		_eSHADER_PIPELINE_FLAG = m_ePIPELINE_STAGE_flags;


	ID3D11DeviceContext* pContext = CONTEXT;
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__VERTEX)
	{
		pContext->VSSetConstantBuffers(m_iRegisterNum, 1, m_CB.GetAddressOf());
	}	
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__HULL)
	{
		pContext->HSSetConstantBuffers(m_iRegisterNum, 1, m_CB.GetAddressOf());
	}	
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__DOMAIN)
	{
		pContext->DSSetConstantBuffers(m_iRegisterNum, 1, m_CB.GetAddressOf());
	}	
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__GEOMETRY)
	{
		pContext->GSSetConstantBuffers(m_iRegisterNum, 1, m_CB.GetAddressOf());
	}	
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__PIXEL)
	{
		pContext->PSSetConstantBuffers(m_iRegisterNum, 1, m_CB.GetAddressOf());
	}
	if (_eSHADER_PIPELINE_FLAG & def_Shader::eSHADER_PIPELINE_STAGE::__COMPUTE)
	{
