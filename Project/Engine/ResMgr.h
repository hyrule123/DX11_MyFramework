...
private:
    unordered_map<string, Ptr<Res>, tHasher_String, std::equal_to<>> m_arrRes[(UINT)eRES_TYPE::END];
...
private:
    ...
    // 
    //함수 설명:
    //위 함수는 T가 C_ComputeShader일 경우에 호출되고
    //아래 함수는 T가 C_ComputeShader가 아닐 경우에 호출됨.
    //같은 표현인데 느낌표만 붙인거임
    //SFINAE를 이용해서 한 쪽으로 캐스팅이 실패하면 다른 오버로딩 함수가 호출되도록 결정 됨.
    //template<typename T>
    //typename std::enable_if<std::is_base_of<C_ComputeShader, T>::value>::type
    //     initRes(Ptr<T>& _Res);

    //template <typename T, typename = std::enable_if_t<!std::is_base_of<C_ComputeShader, T>::value>>
    //   void initRes(Ptr<T>& _Res);

    
public:
    //Ptr<Res> Load(eRES_TYPE _eResType, const std::filesystem::path& _fileName);
    ...
    unordered_map<string, Ptr<Res>, tHasher_String, std::equal_to<>> const& GetResMap(eRES_TYPE _ResType);

    Ptr<Texture> CreateTexture(const string_view _strKey, UINT _uWidth, UINT _uHeight, DXGI_FORMAT _PixelFormat, UINT _D3D11_BIND_FLAG, D3D11_USAGE _Usage);
};

template<typename T>
inline eRES_TYPE ResMgr::GetResType()
{
    static_assert(std::is_base_of_v<Res, T>);
    ...
    else if constexpr (std::is_base_of_v<Anim2DAtlas, T>)
    {
    ...
    else if constexpr (std::is_base_of_v<C_ComputeShader, T>)
    {
...
template<typename T>
inline Ptr<T> ResMgr::FindRes(const string_view _strKey)
{
...
template<typename T>
inline void ResMgr::DeleteRes(const string_view _strKey)
{
...
template<typename T>
inline void ResMgr::AddRes(const string_view _strKey, Ptr<T>& _Res)
{
    static_assert(std::is_base_of<Res, T>::value, "Type T is not Derived class of Res!!");
...
template<typename T>
inline Ptr<T> ResMgr::Load(const std::filesystem::path& _fileName, const string_view _strKey)
{
    //Res를 상속받는 클래스가 아닐 경우 컴파일 중지
    static_assert(std::is_base_of<Res, T>::value);
...
inline unordered_map<string, Ptr<Res>, tHasher_String, std::equal_to<>> const& ResMgr::GetResMap(eRES_TYPE _ResType)
{
    return m_arrRes[(UINT)_ResType];
}

inline Ptr<Texture> ResMgr::CreateTexture(const string_view _strKey, UINT _uWidth, UINT _uHeight, DXGI_FORMAT _PixelFormat, UINT _D3D11_BIND_FLAG, D3D11_USAGE _Usage)
{
