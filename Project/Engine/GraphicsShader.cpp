...
bool CGraphicsShader::SaveJson(Json::Value* _jsonVal)
{
	...
	string comment = "//Enumeration Values are at define.h of cEngine project";
	...
	try
	{
		...
		for (int i = 0; i < (int)define_Shader::eGS_TYPE::END; ++i)
		{
			...
			std::filesystem::path shaderPath = 
				PathMgr::GetInst()->GetPathRel_Content() / DIRECTORY_NAME::SHADER_Bin / sFileName;
...
void CGraphicsShader::BindData()
{
	...
	//Set Rasterizer
	pContext->RSSetState(Device::GetInst()->GetRSState(m_eRSType));

	//Set Output Merger(Depth Stencil, Blend)
	pContext->OMSetDepthStencilState(Device::GetInst()->GetDSState(m_eDSType), 0);
	pContext->OMSetBlendState(Device::GetInst()->GetBSState(m_eBSType), Vec4(0.f, 0.f, 0.f, 0.f), UINT_MAX);
}
