...
Animator2D::Animator2D()
    : Component(eCOMPONENT_TYPE::ANIMATOR2D)
    //, m_uCurFrame()
    , m_iCurFrameIdx()
    , m_fCurTime()
    , m_bFinish()
...
bool Animator2D::SaveJson(Json::Value* _pJVal)
{
    if (nullptr == _pJVal)
        return false;
...
bool Animator2D::LoadJson(Json::Value* _pJVal)
{
    if (nullptr == _pJVal)
        return false;
    else if (false == Component::SaveJson(_pJVal))
    ...
    {
        {
            ...
            for (int i = 0u; i < MAXNUM_ANIM; ++i)
            {
                ...
                Ptr<Anim2DAtlas> pAtlas = ResMgr::GetInst()->Load<Anim2DAtlas>(arrAtlasTex[i].asString());
...
void Animator2D::AddAtlasTex(eMTRLDATA_PARAM_TEX _eTexParam, Ptr<Anim2DAtlas> _pAtlasTex)
{
...
const tAnim2D* Animator2D::FindAnim(const string_view _strKey_Anim)
{
...
bool Animator2D::Play(const string_view _strKey_Anim, eANIM_PLAYMODE _ePlayMode, bool _bReverse)
{
...
bool Animator2D::PreparePlay(const string_view _strKey_Anim, eANIM_PLAYMODE _ePlayMode, bool _bReverse)
{
    const tAnim2D* pAnim = nullptr;
...
const Vec2 Animator2D::GetCurFrameSize() 
{
    Vec2 size;
...
void Animator2D::UpdateAtlasTexToMtrl()
{
    if (false == m_bNeedUpdateMtrl)
        return;

    Material* pMtrl = GetOwner()->RenderComponent()->GetCurMaterial().Get();
...
void Animator2D::CalculateDirectionalColHalfFlipAtlas()
{
