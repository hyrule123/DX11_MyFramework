...
public:
    ...
    Anim2DAtlas(const Anim2DAtlas& _other) = default;
    CLONE_DISABLE(Anim2DAtlas);

    virtual ~Anim2DAtlas();


public:
    //Texture과 같은 key를 사용, 확장자만 변경해서 사용한다.
...
public:
    //1. 아틀라스 텍스처 등록
    void SetAtlasTexture(Ptr<Texture> _AtlasTex);
    Ptr<Texture> GetAtlasTex() const { return m_AtlasTex; }
