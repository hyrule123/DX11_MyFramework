...
Collider::Collider(eCOMPONENT_TYPE _ComType, eDIMENSION_TYPE _eDim)
	: Component(_ComType)
...
bool Collider::SaveJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
	else if (false == Component::SaveJson(_pJson))
		return false;
...
bool Collider::LoadJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
	else if (false == Component::LoadJson(_pJson))
		return false;
...
void Collider::BeginCollision(Collider* _other, const Vec3& _v3HitPoint)
{
	++m_iCollisionCount;
	_other->AddCollisionCount();

	ScriptHolder* pSH = GetOwner()->ScriptHolder();
	if (nullptr != pSH)
...
void Collider::OnCollision(Collider* _other, const Vec3& _v3HitPoint)
{
	ScriptHolder* pSH = GetOwner()->ScriptHolder();
	if (nullptr != pSH)
...
void Collider::EndCollision(Collider* _other)
{
	--m_iCollisionCount;
