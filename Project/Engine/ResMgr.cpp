...
void ResMgr::CreateDefaultShader()
{
...
bool ResMgr::CreateDefaultGraphicsShader()
{
	{
		// ============
		// Debug Shader
...
bool ResMgr::CreateDefaultComputeShader()
{
	//컴퓨트쉐이더는 클래스에 종속적이므로 수동으로 로드해줘야 한다.
	try
	{
		Ptr<CCS_Initialize> pCS = ResMgr::GetInst()->Load<CCS_Initialize>(strKey_RES_DEFAULT::SHADER::COMPUTE::INITALIZE);
	...
	try
	{
		Ptr<CCS_SetColor> pCS = ResMgr::GetInst()->Load<CCS_SetColor>(strKey_RES_DEFAULT::SHADER::COMPUTE::SETCOLOR);
		pCS->SetEngineDefaultRes(true);
	}
	catch (const std::runtime_error& error)
	{
	...
	try
	{
		Ptr<CCS_ParticleBasic> pCS = ResMgr::GetInst()->Load<CCS_ParticleBasic>(strKey_RES_DEFAULT::SHADER::COMPUTE::PARTICLEBASIC);
		pCS->SetEngineDefaultRes(true);
	}
	catch (const std::runtime_error& error)
	{
...
void ResMgr::CreateDefaultMaterial()
{
	//Debug Material
	{
		//Rectangle
		Ptr<Material> pMtrl = new Material;
...
void ResMgr::LoadDefaultTexture()
{
	Load<Texture>(strKey_RES_DEFAULT::TEXTURE::DEFAULT);
	Load<Texture>(strKey_RES_DEFAULT::TEXTURE::NOISE_1);
}

bool ResMgr::LoadUserGraphicsShaderAll()
{
	std::filesystem::path GSDir(PathMgr::GetInst()->GetPathRel_Resource(eRES_TYPE::GRAPHICS_SHADER));
	...
	for (; false == GSDirIter._At_end(); ++GSDirIter)
	{
		if (false == GSDirIter->is_directory())
		{
			...
			if (string::npos != fileName.find(RES_INFO::SHADER::Ext_ShaderSetting))
			{
				...
				ResMgr::GetInst()->AddRes<CGraphicsShader>(pGS->GetKey(), pGS);
			}
		}
	}
...
//Ptr<Res> ResMgr::Load(eRES_TYPE _eResType, const std::filesystem::path& _fileName)
//{
//	Ptr<Res> pRes;
//
//	switch (_eResType)
