...
void RenderMgr::RegisterCamera(Camera* _pCam, eCAMERA_INDEX _idx)
{
    assert(0 <= (int)_idx && (int)_idx < (int)eCAMERA_INDEX::END);
    m_arrCam[(int)_idx] = _pCam;
}

void RenderMgr::RemoveCamera(Camera* _pCam)
{
...
void RenderMgr::UpdateDebugShapeRender(vector<tDebugShapeInfo>& _vecDebugRef)
{
...
void RenderMgr::UpdateBuffer()
{
    //글로벌 정보를 tGlobalValue 상수버퍼로 업데이트
    cConstBuffer* pConstBuffer = Device::GetInst()->GetConstBuffer(REGISLOT_b_CBUFFER_GLOBAL);
...
{
    //카메라의 행렬을 상수버퍼에 업로드
    static cConstBuffer* const pBuffer = Device::GetInst()->GetConstBuffer(REGISLOT_b_CBUFFER_CAM_MATIRCES);
...
void RenderMgr::AddInstancingQueue(const tInstancingKey& _Key, const tMtrlScalarData& _Value)
{
    m_mapInstancing[_Key].push_back(_Value);
}

void RenderMgr::InstancedRender()
{
    for (const auto& iter : m_mapInstancing)
    {
        ...
        Material* pMtrl = (Material*)(iter.first.pMtrl);
        pMtrl->BindData();

        Mesh* pMesh = (Mesh*)(iter.first.pMesh);
