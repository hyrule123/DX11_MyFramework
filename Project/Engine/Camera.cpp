...
Camera::Camera():
	Component(eCOMPONENT_TYPE::CAMERA)
	, m_AspectRatio()
...
void Camera::SetProjType(ePROJ_TYPE _Type)
{
...
void Camera::SetCamIndex(eCAMERA_INDEX _Idx)
{
	assert(_Idx <= eCAMERA_INDEX::END);

	m_iCamIdx = (int)_Idx;
	RenderMgr::GetInst()->RegisterCamera(this, _Idx);
}



void Camera::Zoom2D(float _fScale)
{
...
void Camera::cleanup()
{
	RenderMgr::GetInst()->RemoveCamera(this);
}


void Camera::SortObject()
{
	RenderMgr* pRenderMgr = RenderMgr::GetInst();
	...
	for (UINT32 i = 0; i < MAX_LAYER; ++i)
	{
		...
		Layer& Layer = LevelMgr::GetInst()->GetCurLevel()->GetLayer(i);

		//카메라가 출력하고자 하는 레이어의 오브젝트 리스트를 받아와서
		const vector<GameObject*>& vecObj = Layer.GetvecObj();
		...
		{
			//출력 담당 컴포넌트를 받아온다.
			RenderComponent* pRenderCom = vecObj[i]->RenderComponent();
			...
			//도메인이 등록되어 있을 경우 RenderMgr의 출력 대기열에 등록한다.
