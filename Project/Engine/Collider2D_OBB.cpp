...
Collider2D_OBB::Collider2D_OBB()
	: Collider2D(eCOLLIDER_TYPE_2D::OBB)
	, m_tOBBInfo{}
{
}

Collider2D_OBB::Collider2D_OBB(const Collider2D_OBB& _other)
	: Collider2D(_other)
{
}
...
//이 함수는 Transform에서 값이 변했을 경우에만 호출된다.
void Collider2D_OBB::UpdateCollider()
{
	//충돌체 주소를 가져온다.
	const Transform& pTransform = GetOwner()->Transform();
...
void Collider2D_OBB::UpdateSimpleCollider(Vec4& _vSimpleCollLBRTPos)
{
...
void Collider2D_OBB::DebugRender()
{
	//충돌체(자신)의 정보)
