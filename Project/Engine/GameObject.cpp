...
GameObject::~GameObject()
{
	//Layer에 등록된 GameObject*의 경우는 EventMgr을 통해서 제거 할시 알아서 제거됨.
...
void GameObject::SetMtrlScalarParam(eMTRLDATA_PARAM_SCALAR _Param, const void* _Src)
{
	switch (_Param)
	{
...
bool GameObject::render()
{
	...
	ScriptHolder* pHolder = ScriptHolder();
	if (pHolder)
	{
		pHolder->BindData();
	}
...
bool GameObject::SaveJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
	else if (false == Entity::SaveJson(_pJson))
		return false;
...
bool GameObject::LoadJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
		...
		for (int i = 0; i < arrComSize; ++i)
		{
			...
			case eCOMPONENT_TYPE::COLLIDER2D:
			{
				...
				case eCOLLIDER_TYPE_2D::RECT:
					pCom = new Collider2D_Rect;
					break;
				case eCOLLIDER_TYPE_2D::CIRCLE:
					pCom = new Collider2D_Circle;
					break;
				case eCOLLIDER_TYPE_2D::OBB:
					pCom = new Collider2D_OBB;
					break;
				case eCOLLIDER_TYPE_2D::POINT:
					pCom = new Collider2D_Point;
					break;
				default:
					break;
				}
			...
			case eCOMPONENT_TYPE::TILEMAP:
			{
				...
				if (jsonComponent.isMember(strKey))
				{
					...
					case eTILEMAP_TYPE::ATLAS:
						pCom = new TilemapAtlas;
						break;
					case eTILEMAP_TYPE::COMPLETE:
						pCom = new TilemapComplete;
						break;
					default:
	...
	//계층 프리팹 로드
	{
		...
		{
			Ptr<CPrefab> pPrefab = ResMgr::GetInst()->Load<CPrefab>(iter->asString());
			if (nullptr == pPrefab)
			{
...
void GameObject::AddComponent(Component* _Component)
{
	...
	case eCOMPONENT_TYPE::DECAL:
		...
		m_RenderCom = static_cast<RenderComponent*>(_Component);
		break;
...
void GameObject::RemoveComponent(eCOMPONENT_TYPE _eComType)
{
...
void GameObject::AddScript(Script* _Script)
{
	if (nullptr == _Script)
		return;

	ScriptHolder* pScriptHolder = static_cast<ScriptHolder*>(m_arrCom[(UINT)eCOMPONENT_TYPE::SCRIPT_HOLDER]);

	if (nullptr == pScriptHolder)
	{
		pScriptHolder = new ScriptHolder;
		AddComponent(pScriptHolder);
	}
	pScriptHolder->AddScript(_Script);
}

void GameObject::AddChildGameObj(GameObject* _Object)
{
...
void GameObject::SetParentMatrixUpdated()
{
...
bool GameObject::GetParentWorldMatrix(Matrix& _mat)
{
