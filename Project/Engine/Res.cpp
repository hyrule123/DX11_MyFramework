...
bool Res::Save(const std::filesystem::path& _fileName)
{
...
bool Res::SaveJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
...
bool Res::Load(const std::filesystem::path& _fileName)
{
	//ResType을 인덱스로 써서 상대경로를 받아올 수 있다.
	std::filesystem::path FilePath = GETRESPATH;
	FilePath /= _fileName;
...
bool Res::LoadJson(Json::Value* _pJson)
{
	if (nullptr == _pJson)
		return false;
