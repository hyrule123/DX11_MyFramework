...
Anim2DAtlas::Anim2DAtlas()
	: Res(eRES_TYPE::ANIM2D_ATLAS)
	, m_uColTotal()
	, m_uRowTotal()
	, m_bRegularFrameSize()
{
...
//bool Anim2DAtlas::Save(const std::filesystem::path& _fileName)
//{
...
//}
//
//bool Anim2DAtlas::Load(const std::filesystem::path& _fileName)
//{
//
//}
...
bool Anim2DAtlas::Load(const std::filesystem::path& _fileName)
{
	...
	return Res::Load(AddExt);
}

bool Anim2DAtlas::SaveJson(Json::Value* _jVal)
{
	if (nullptr == _jVal)
		return false;
	else if (false == Res::SaveJson(_jVal))
		return false;
	...
	jVal[JsonKey_Anim2DAtlas::vecFrameUV] = Json::Value(Json::ValueType::objectValue);
	{
		Json::Value& FrameUV = jVal[JsonKey_Anim2DAtlas::vecFrameUV];
		FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVLeftTop] = Json::Value(Json::arrayValue);
		FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVSlice] = Json::Value(Json::arrayValue);
		FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_Offset] = Json::Value(Json::arrayValue);
		for (size_t i = 0; i < m_vecFrameUV.size(); ++i)
		{
			...
			FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVLeftTop].append(Pack_v2_i64(Frame.v2_UVLeftTop).i64);

			FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVSlice].append(Pack_v2_i64(Frame.v2_UVSlice).i64);

			FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_Offset].append(Pack_v2_i64(Frame.v2_Offset).i64);
		}
	}

	{
		jVal[JsonKey_Anim2DAtlas::mapAnim] = Json::Value(Json::objectValue);
		Json::Value& mapAnim = jVal[JsonKey_Anim2DAtlas::mapAnim];
...
bool Anim2DAtlas::LoadJson(Json::Value* _jVal)
{
	if (nullptr == _jVal)
		return false;
	else if (false == Res::LoadJson(_jVal))
		return false;

	const Json::Value& jVal = *_jVal;

	if (jVal.isMember(JsonKey_Anim2DAtlas::strKeyAtlasTex))
	{
		const string& strKey = jVal[JsonKey_Anim2DAtlas::strKeyAtlasTex].asString();
		Ptr<Texture> tex = ResMgr::GetInst()->Load<Texture>(strKey);
		if (nullptr == tex)
		{
	...
	if (jVal.isMember(JsonKey_Anim2DAtlas::bRegularFrameSize))
	{
		m_bRegularFrameSize = jVal[JsonKey_Anim2DAtlas::bRegularFrameSize].asBool();
	}

	if (jVal.isMember(JsonKey_Anim2DAtlas::vecFrameUV))
	{
		const Json::Value& FrameUV = jVal[JsonKey_Anim2DAtlas::vecFrameUV];
		if (
			FrameUV.isMember(JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVLeftTop)
			&&
			FrameUV.isMember(JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVSlice)
			&&
			FrameUV.isMember(JsonKey_Anim2DAtlas::AnimFrameUV::v2_Offset)
			)
		{
			const Json::Value& v2_UVLeftTop = FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVLeftTop];
			const Json::Value& v2_UVSlice = FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_UVSlice];
			const Json::Value& v2_Offset = FrameUV[JsonKey_Anim2DAtlas::AnimFrameUV::v2_Offset];
	...
	{
		if (jVal.isMember(JsonKey_Anim2DAtlas::mapAnim))
		{
			const Json::Value& mapAnim = jVal[JsonKey_Anim2DAtlas::mapAnim];
...
void Anim2DAtlas::SetAtlasTexture(Ptr<Texture> _AtlasTex)
{
...
void Anim2DAtlas::SetNewAnimUV(vector<tAnimFrameUV>& _vecFrameUV)
{
	std::swap(m_vecFrameUV, _vecFrameUV);
}

void Anim2DAtlas::SetNewAnimUV(UINT _uColTotal, UINT _uRowTotal)
{
...
void Anim2DAtlas::SetNewAnimUV(UINT _uColTotal, UINT _uRowTotal, UINT _uColStart, UINT _uColPitch, UINT _uRowStart, UINT _uRowPitch)
{
	if (nullptr == m_AtlasTex)
		return;
...
void Anim2DAtlas::SetNewAnimUV_SC_Redundant(UINT _uRowTotal, UINT _uRowStart, UINT _uRowPitch)
{
	if (nullptr == m_AtlasTex)
		return;
...
tAnim2D* Anim2DAtlas::AddAnim2D(const string_view _strAnimKey, const tAnim2D& _vecAnimFrameIdx,
...
tAnim2D* Anim2DAtlas::AddAnim2D(const string_view _strAnimKey, const vector<UINT>& _vecFrame, float _fFullPlayTime, eANIM_TYPE _eAnimType, Vec2 _vPivot)
{
	tAnim2D frameIdx = {};
...
tAnim2D* Anim2DAtlas::AddAnim2D(const string_view _strAnimKey, UINT _uColStart, UINT _uColPitch, UINT _uRowStart, UINT _uRowPitch,
	float _fFullPlayTime, eANIM_TYPE _eAnimType, Vec2 _vPivot
)
{
...
tAnim2D* Anim2DAtlas::AddAnim2D_SC_Redundant(const string_view _strAnimKey, UINT _uRowStart, UINT _uRowPitch, float _fFullPlayTime, Vec2 _vPivot)
{
	assert(
		0 < _uRowPitch 
...
//
tAnim2D* Anim2DAtlas::AddAnim2D_vecRowIndex(const string_view _strAnimKey, const vector<UINT>& _vecRow, float _fFullPlayTime, Vec2 _vPivot)
{
	tAnim2D Anim = {};
...
const tAnim2D* Anim2DAtlas::FindAnim2D(const string_view _AnimIdxStrKey)
{
...
Vec2 Anim2DAtlas::GetFrameSize(UINT _uIdxFrameUV) const
{
