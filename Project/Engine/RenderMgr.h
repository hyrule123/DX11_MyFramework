...
private:
	Camera* m_arrCam[(UINT)eCAMERA_INDEX::END];
...
public:
	//inline getter
	Camera* GetCamera(eCAMERA_INDEX _iCamIdx) { return m_arrCam[(int)_iCamIdx]; }
	Camera* GetCurCamera();

	void RegisterCamera(Camera* _pCam, eCAMERA_INDEX _idx);
	void RemoveCamera(Camera* _pCam);
	...
	void SetEditorCam(Camera* _pCamera) { m_arrCam[(int)eCAMERA_INDEX::EDITOR] = _pCamera; }
...
inline void RenderMgr::AddDebugShapeRender(const tDebugShapeInfo& _tDebugShapeInfo)
{
...
inline Camera* RenderMgr::GetCurCamera()
{
	if (m_bEditorCamMode)
...
inline void RenderMgr::AddRenderQueue(tRenderInfo _pRenderCom, define_Shader::eSHADER_DOMAIN _eShaderDomain)
{
