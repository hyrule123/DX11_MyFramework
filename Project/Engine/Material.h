...
public:
    ...
    Material(const Material& _Clone);
    CLONE(Material)
public:
...
private:
    ...
    Ptr<Texture>           m_arrTex[(int)eMTRLDATA_PARAM_TEX::_END];
...
public:
    void SetTexParam    (eMTRLDATA_PARAM_TEX _Param, Ptr<Texture> _Tex);
    ...
    Ptr<Texture> GetTexture(eMTRLDATA_PARAM_TEX _texIdx = eMTRLDATA_PARAM_TEX::_0) const;

private:
    bool IsTexture(eMTRLDATA_PARAM_TEX _Idx);
};

inline Ptr<Texture> Material::GetTexture(eMTRLDATA_PARAM_TEX _texIdx) const
{
    return m_arrTex[(UINT)_texIdx];
}


inline bool Material::IsTexture(eMTRLDATA_PARAM_TEX _Idx)
{
    switch (_Idx)
    {
...
inline void Material::RemoveTexture(eMTRLDATA_PARAM_TEX _Param)
{
