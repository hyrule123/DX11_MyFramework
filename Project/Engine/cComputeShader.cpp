...
{
	if(m_pShaderModule.get())
		m_pShaderModule->BindData();

	//컴퓨트쉐이더 관련 공유 데이터를 상수버퍼를 통해서 전달
	cConstBuffer* pCBuffer = cDevice::GetInst()->GetConstBuffer(REGISLOT_b_CBUFFER_MTRL_SCALAR);
...
bool cComputeShader::SaveJson(Json::Value* _jsonVal)
{
	...
	{
		jVal[JsonKey_cComputeShader::uarrNumThreadXYZ].append(m_arrNumThread[i]);
	}

	if (m_pShaderModule.get())
	{
		jVal[JsonKey_cComputeShader::ShaderModule] = Json::Value(Json::objectValue);
		m_pShaderModule->SaveJson(&jVal[JsonKey_cComputeShader::ShaderModule]);
	}
...
bool cComputeShader::LoadJson(Json::Value* _jsonVal)
{
	...
	if (true == _jsonVal->isMember(JsonKey_cComputeShader::uarrNumThreadXYZ))
	{
		if (NumAxis == (*_jsonVal)[JsonKey_cComputeShader::uarrNumThreadXYZ].size())
		{
			...
			{
				m_arrNumThread[i] = (*_jsonVal)[JsonKey_cComputeShader::uarrNumThreadXYZ][i].asInt();
			}
		}
	}
...
void cComputeShader::CalcGroupNumber(UINT _ElemCountX, UINT _ElemCountY, UINT _ElemCountZ)
{
	...
	m_uNumGroupArr[X] = (UINT)ceilf((float)_ElemCountX / (float)m_arrNumThread[X]);
	m_uNumGroupArr[Y] = (UINT)ceilf((float)_ElemCountY / (float)m_arrNumThread[Y]);
	m_uNumGroupArr[Z] = (UINT)ceilf((float)_ElemCountZ / (float)m_arrNumThread[Z]);
...
bool cComputeShader::AddShaderModule(std::unique_ptr<cShaderModule>&& _pShaderModule)
{
	assert(_pShaderModule.get());
	m_pShaderModule = std::move(_pShaderModule); 
	m_pShaderModule->SetOwner(this);
	return m_pShaderModule->Init();
}
