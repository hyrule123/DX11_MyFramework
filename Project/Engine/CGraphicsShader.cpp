...
CGraphicsShader::CGraphicsShader()
	...
	, m_eRSType(def_Shader::eRASTERIZER_TYPE::CULL_BACK)
	, m_eDSType(def_Shader::eDEPTH_STENCIL_TYPE::LESS)
	, m_eBSType(def_Shader::eBLEND_STATE_TYPE::DEFAULT)
	, m_eShaderDomain(def_Shader::eSHADER_DOMAIN::_UNDEFINED)
	, m_arrShaderCode{}
	, m_bUseInstancing()
{
}
...
bool CGraphicsShader::SaveJson(Json::Value* _jsonVal)
{
	...
	for (int i = 0; i < (int)def_Shader::eSHADER_TYPE::END; ++i)
	{
		...
		switch ((def_Shader::eSHADER_TYPE)i)
		{
		case def_Shader::eSHADER_TYPE::__VERTEX:
			if (nullptr != m_VS.Get())
				bExist = true;
			break;
		case def_Shader::eSHADER_TYPE::__HULL:
			if(nullptr != m_HS.Get())
				bExist = true;
			break;
		case def_Shader::eSHADER_TYPE::__DOMAIN:
			if (nullptr != m_DS.Get())
				bExist = true;
			break;
		case def_Shader::eSHADER_TYPE::__GEOMETRY:
			if (nullptr != m_GS.Get())
				bExist = true;
			break;
		case def_Shader::eSHADER_TYPE::__PIXEL:
			if (nullptr != m_PS.Get())
				bExist = true;
			break;
	...
	{
		{
			...
			m_eBSType = (def_Shader::eBLEND_STATE_TYPE)jVal[strKey].asInt();
		}

		{
			string strKey = RES_INFO::SHADER::GRAPHICS::JSON_KEY::eDEPTH_STENCIL_TYPE;
			CHECK_LOAD_ERROR(strKey);
			m_eDSType = (def_Shader::eDEPTH_STENCIL_TYPE)jVal[strKey].asInt();
		}

		{
			string strKey = string(RES_INFO::SHADER::GRAPHICS::JSON_KEY::eRASTERIZER_TYPE);
			CHECK_LOAD_ERROR(strKey);
			m_eRSType = (def_Shader::eRASTERIZER_TYPE)jVal[strKey].asInt();
		}
...

		{
			...
			CHECK_LOAD_ERROR(strKey);
			m_eShaderDomain = (def_Shader::eSHADER_DOMAIN)jVal[strKey].asInt();
		}


		if (def_Shader::eSHADER_DOMAIN::_UNDEFINED == m_eShaderDomain)
		{
		...
		for (int i = 0; i < (int)def_Shader::eSHADER_TYPE::END; ++i)
		{
			...
			if (sFileName.empty())
			{
				...
				if ((int)def_Shader::eSHADER_TYPE::__VERTEX == i)
				{
			...
			//실패시 동적할당 해제하고 오류 발생
			hr = CreateShader(sCode, (def_Shader::eSHADER_TYPE)i);
			if (FAILED(hr))
			{
				throw(std::runtime_error(string("Shader Blob Creation Failed.\nError Code: ") + std::to_string(hr)));
			}
...
HRESULT CGraphicsShader::CreateDefaultInputLayout()
{
	...
	const tShaderCode& shaderCode = m_arrShaderCode[(int)def_Shader::eSHADER_TYPE::__VERTEX];
...
HRESULT CGraphicsShader::CreateShaderFromHeader(const unsigned char* _pByteCode, size_t _ByteCodeSize, def_Shader::eSHADER_TYPE _eShaderType)
{
...
HRESULT CGraphicsShader::CreateShaderPrivate(const void* _pByteCode, size_t _ByteCodeSize, def_Shader::eSHADER_TYPE _ShaderType)
{
...
HRESULT CGraphicsShader::CreateShader(const std::filesystem::path& _FileName, const string_view _strFuncName, def_Shader::eSHADER_TYPE _ShaderType)
{
	if (GetKey().empty())
	...
	default:
		//에러 발생시킴
		assert((int)def_Shader::eSHADER_TYPE::END == 0);
		break;
	}
