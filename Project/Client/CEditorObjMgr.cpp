...
void CEditorObjMgr::CreateDebugShape()
{
	...
	for (int i = 0; i < (int)eDEBUGSHAPE_TYPE::END; ++i)
	{
		...
		case eDEBUGSHAPE_TYPE::RECT:
		{
			...
			Ptr<CMesh> pDebugMesh = CResMgr::GetInst()->FindRes<CMesh>(RESOURCES::MESH::DEBUG_RECT);
			Ptr<CMaterial> pDebugMtrl = CResMgr::GetInst()->FindRes<CMaterial>(RESOURCES::MATERIAL::DEBUG_RECT);
		...
		{
			CMeshRender* pMesh = new CMeshRender;
			Ptr<CMesh> pDebugMesh = CResMgr::GetInst()->FindRes<CMesh>(RESOURCES::MESH::DEBUG_CIRCLE);
			Ptr<CMaterial> pDebugMtrl = CResMgr::GetInst()->FindRes<CMaterial>(RESOURCES::MATERIAL::DEBUG_CIRCLE);
			pMesh->SetMesh(pDebugMesh);
			pMesh->SetMaterial(pDebugMtrl);
			m_arrDebugShape[i]->AddComponent(pMesh);
			break;
